...
**Q: CSV 导出功能如何实现？**
- A: 导出入口在 `AdminView.exportBusinessesFlow` 与 `FoodService.exportFoods`。
- 先读取导出文件名（回车使用默认名），再拼接到 `csvData/` 目录。
- 调用 DAO 获取列表数据，组装表头 + 数据行的 `List<String[]>`。
- 调用 `CsvUtil.writeCsv(Path, rows)` 写入文件：自动创建目录、写入 UTF-8 BOM，并逐行输出（含逗号/引号/换行的转义）。

**Q: CSV 导出为什么能保证 Excel 不乱码？**
- A: `CsvUtil` 写入 UTF-8 BOM（字节序标记），Windows 版 Excel 会据此识别 UTF-8 编码。
- BOM 是文件开头的三个字节 `EF BB BF`；没有 BOM 时 Excel 可能按系统默认编码（如 GBK）读取，导致中文乱码。

**Q: 分页功能是怎么实现的？**
- A: `FoodService.listFoods` 先查询商家食品列表，数量小于等于 `PAGE_SIZE` 时直接打印。
- 若数据量较大则进入 `paginateFoods`，计算总页数并用 `subList(start, end)` 取当前页。
- 控制台提供“上一页/下一页/跳转页码/退出”，在边界页会提示“已经是第一页/最后一页”。
...
**Q: 你认为系统还有哪些可以优化？**
- A: 增加排序与筛选；密码加密存储；输入更严格校验；统一日志；权限控制；将分页下推到数据库。

**Q: 如果数据量很大，当前实现会有什么问题？**
- A: 目前先全量查询再在内存分页展示，数据量大时仍会卡顿；可改为 SQL 分页、条件过滤与索引优化。
